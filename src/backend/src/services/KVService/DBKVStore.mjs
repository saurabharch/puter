...
export class DBKVStore {
    ...
    async incr({ key, path = '', amount = 1 }) {
        return await this.#increment({ key, path, amount: Math.abs(amount) });
    }
    async decr({ key, path = '', amount = 1 }) {
        return await this.#increment({ key, path, amount: -Math.abs(amount) });
    }
    async #increment({ key, path, amount }) {
        const currVal = this.get(key);
        if ( typeof currVal === 'number' ){
            // simply set with increment
            this.set(key, currVal + amount);
        } else if ( typeof currVal === 'object' ){
            const pathKeys = path.split('.');
            let val = currVal;
            for ( const pathKey of pathKeys.slice(0, -1) ){
                val[pathKey];
                if ( !val ){
                    break;
                }
            }
            if ( typeof val[pathKeys.at(-1)]  === 'number' ){
                val[pathKeys.at(-1)] += amount;
            }
            this.set(key, val);
        }
        throw APIError.create('field_invalid', null, {
            key: 'path',
            expected: 'key and path containing a number',
        });
    }
    async #expireat(key, timestamp) {
        const actor = Context.get('actor');
